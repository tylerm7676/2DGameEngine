...
bool RectangularCollisionStrategy::execute(const Drawable& sprite1,
  const Drawable& sprite2) const
{
...
float MidPointCollisionStrategy::distance(float x1, float y1, float x2,
  float y2) const
{
...
bool MidPointCollisionStrategy::execute(const Drawable& sprite1,
  const Drawable& sprite2) const
{
...
bool PerPixelCollisionStrategy::isVisible(Uint32 pixel,
...
SDL_Surface* scaleSurface(const SDL_Surface* const surf, int width, int height)
{
  ...
  if(flag < 0)
    throw(std::string("SDL_BlitScaled failed!"));
...
bool PerPixelCollisionStrategy::execute(const Drawable& obj1,
  const Drawable& obj2) const
{
